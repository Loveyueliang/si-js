 ...
 * Aip音乐播放器
 * author: Xvites/Zing
 * qq: 3074193836/8589561
 * version: 2.1.0
 * webSite: https://www.toubiec.cn
 * time: 2019/03/21
...
var lzxPlayerInit = function () {
    ...
    if(!isTest){
        ...
        if (top.location !== self.location || isLoad) {
           return console.log('取消加载播放器.原因：其他页面已加载')
        }
    ...
    $("body").append('<div id="lzxPlayer">\n' +
        ...
        '                    <span class="itime">00:00 / 00:00</span></span>\n' +
        '            </div>\n' +
        ...
        '            </b>\n' +
        '            <div id="div1" class="inote">\n' +
        '                <i class="fa fa-music" aria-hidden="true"></i>\n' +
        '            </div>\n' +
        '            <div id="div2" class="inote">\n' +
        '                <i class="fa fa-music" aria-hidden="true"></i>\n' +
        '            </div>\n' +
        '            <div id="div3" class="inote">\n' +
        '                <i class="fa fa-music" aria-hidden="true"></i>\n' +
        '            </div>\n' +
        '        </div>\n' +
        ...
        '            <div class="rate">\n' +
        '                <div class="progress">\n' +
        '                    <div class="irate-buffered"></div>' +
        '                    <div class="irate-on" style="width: 0;">\n' +
        '                        <div class="drag"></div>\n' +
        '                    </div>\n' +
        '                </div>\n' +
        '            </div>\n' +
    ...
    var audio = new Audio(),
        ...
        $rateBuffered = $('.musicbottom .irate-buffered', $player);
    ...
    $cover.html('<img src="https://tva1.sinaimg.cn/large/005BYqpggy1g428f6my70j302n02ja9u.jpg">');
    $songName.html('<a style="color:#f00">正在初始化</a>');
    $songFrom.html('<a style="color:#f00">aipcode</a>');
    ...
    var lzxMedia = {
        play: function () {
            ...
            cicleTime = setInterval(function(){
                if(!rateMouse.isDown && !rateTouch.isTouchDown){
                    ...
                    $(".irate-on",$rateSlider).width(audio.currentTime / audio.duration * 100+"%");
                }
        ...
        seeking: function () {
            var volde = window.parseInt(audio.currentTime / audio.duration * 100);
            lzxTips.show('音乐当前进度：'+ volde + '%' );
        },
    ...
    $('.drag', $rateSlider).on("touchstart",(function (e) {
        ...
        rateTouch.rateOnWidth = parseFloat(($(".irate-on",$rateSlider).width() / rateTouch.progressWidth).toFixed(2));
    }));
    ...
    $('.drag', $rateSlider).mousedown(function (e) {
        ...
        rateMouse.rateOnWidth = parseFloat(($(".irate-on",$rateSlider).width() / rateMouse.progressWidth).toFixed(2));
    });
    ...
    window.addEventListener("touchmove",function(e){
        if(rateTouch.isTouchDown){
            ...
            if(rate >= 0 && rate <= 1){
                $(".irate-on",$rateSlider).width(rate * 100 + '%');
            ...
            song: function (id, isThisAlbum) {
                ...
                for (var i = 0; i < songTotal; i++) {
                    songList += '<li><span class="index">' + (i + 1) + '</span><span class="artist"></span>'
                        + songSheetList[id].songNames[i] + ' - '+ songSheetList[id].artistNames[i] + '</li>'
                }
    ...
    //获取歌单列表数据
    $.ajax({
        url: webURL + '/api/playerlist?id=' + keyId,
        type: 'GET',
        dataType: 'script',
        success: function () {
            if(playerWidth !== -1){
    ...
    function startPlay() {
        ...
        lzxTips.show('开始从' + songFrom55 + '播放 - ' + songSheetList[albumId].songNames[songId] + ' 时长：'+Math.floor(t/60)+":"+(t%60/100).toFixed(2).slice(-2));
        console.log(siteName + ' - ' + hosts + ' - 当前播放：'+ songSheetList[albumId].songNames[songId] + ' - '
            + songSheetList[albumId].artistNames[songId] + ' 时长：'+Math.floor(t/60)+":"+(t%60/100).toFixed(2).slice(-2));
			}).catch((e) => {
				// 音频加载失败
				lzxTips.show('浏览器限制音频自动播放！请点击播放');
        console.log('浏览器限制音频自动播放！请点击播放');
			});
		}
    }
    ...
    function playerColor() {
        ...
        $(".infos,.control,.status .inote", $player).css({
            color: 'rgb(' + font_color + ')'
        });
    }
